...
/// <summary>
/// Lightweight test host for the ISEStudio web project used by the
...
/// (<c>ISEStudio.Tests.Authentication.AuthTestWebApplicationFactory</c>)
...
/// <c>ISEStudio:Ontology:RocksDbPath</c> configuration value.</para>
/// </summary>
internal sealed class ApiContractWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    private readonly string _sqlitePath = Path.Combine(
        Path.GetTempPath(),
        $"isestudio-contract-{Guid.NewGuid():N}.db");
    private readonly string _rdfRoot = Path.Combine(
        Path.GetTempPath(),
        $"isestudio-contract-rdf-{Guid.NewGuid():N}")
    ...
    private readonly string _blobRoot = Path.Combine(
        Path.GetTempPath(),
        $"isestudio-contract-blob-{Guid.NewGuid():N}");
    ...
    private readonly string _exportRoot = Path.Combine(
        Path.GetTempPath(),
        $"isestudio-contract-exports-{Guid.NewGuid():N}");
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            // scoped to ISEStudio:Ontology:RocksDbPath; here we
...
/// <c>ISEStudio.Tests.Extraction</c> but lives here so the
