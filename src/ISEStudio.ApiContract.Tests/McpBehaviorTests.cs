...
/// <summary>
/// Behavior tests for the ISEStudio MCP transport. These exercise
/// the tool bodies directly (without going through HTTP / JSON-RPC)
/// so a regression in <see cref="ISEStudioMcpTools"/> fails with a
...
public sealed class McpBehaviorTests
{
    ...
    {
        Assert.Equal(20, ISEStudioMcpTools.Inventory().Count);
    }
    ...
    {
        Assert.Equal(50, ISEStudioMcpTools.MaxEditsPerDestructiveCall);
    }
    ...
    {
        Assert.Equal(200 * 1024, ISEStudioMcpTools.MaxDestructivePayloadBytes);
    }
    ...
    {
        var names = ISEStudioMcpTools.Inventory()
            .Select(t => t.Name)
