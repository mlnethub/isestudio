...
///   <item>Always &rarr; <c>X-ISEStudio-Release</c> + a quoted
...
public sealed class PublishedCacheContractTests
{
    ...
    public async Task Current_release_sets_release_and_etag_headers_with_no_cache()
    {
        ...
        // not the right test.
        Assert.Equal("v1", response.Headers.GetValues("X-ISEStudio-Release").Single());
    ...
    public async Task Pinned_release_sets_immutable_cache_control_header()
    {
        ...
        Assert.Equal("v2", response.Headers.GetValues("X-ISEStudio-Release").Single());
