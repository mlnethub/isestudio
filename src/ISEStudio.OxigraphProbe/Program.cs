...
// short report describing every API surface ISEStudio.StoreWrapper depends on.
//
// Usage:  dotnet run --project src/ISEStudio.OxigraphProbe -- <store-directory>
...
var roundTripDir = Path.Combine(Path.GetTempPath(), "isestudio-probe-rt-" + Guid.NewGuid().ToString("N"));
try
{
...
//     shape ISEStudio.StoreWrapper.ReplaceGraph needs to expose.
