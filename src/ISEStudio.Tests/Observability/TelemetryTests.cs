...
public sealed class TelemetryTests
{
    ...
    public void All_source_names_are_registered()
    {
        ...
        Assert.Equal("ISEStudio.Llm", Telemetry.LlmSource.Name);
        Assert.Equal("ISEStudio.Rdf", Telemetry.RdfSource.Name);
        Assert.Equal("ISEStudio.Parsing", Telemetry.ParsingSource.Name);
        Assert.Equal("ISEStudio.Storage", Telemetry.StorageSource.Name);
        Assert.Equal("ISEStudio.Mcp", Telemetry.McpSource.Name);

        // The shared meter is exposed under "ISEStudio" so the
        // .WithMetrics(m => m.AddMeter("ISEStudio")) call in Program.cs
        // picks up every counter / histogram defined here.
        Assert.Equal("ISEStudio", Telemetry.Meter.Name);
    }
    ...
    {
        Assert.Equal("isestudio.extraction.started", Telemetry.ExtractionsStarted.Name);
        Assert.Equal("{chunk}", Telemetry.ExtractionsStarted.Unit);
        Assert.Equal("isestudio.extraction.completed", Telemetry.ExtractionsCompleted.Name);
        Assert.Equal("isestudio.extraction.duration", Telemetry.ExtractionDuration.Name);
