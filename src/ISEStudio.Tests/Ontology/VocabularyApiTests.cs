...
public sealed class VocabularyApiTests
{
        ...
        AuthTestWebApplicationFactory app, Guid ksGuid)
    {
        var service = app.Services
            .GetRequiredService<ISEStudio.Authentication.IKnowledgeApiTokenService>();
        ...
        AuthTestWebApplicationFactory app, Guid ksGuid)
    {
        ...
        var chunk = new ChunkEntity
        {
            ...
            // so the _source_contains grounding check (P3-8) accepts the
            // fake-chat proposals generated by FakeChat.EnqueueTerminologyProposal
            // (which uses those literal labels). Without this, the proposals
            // would all be dropped silently and the test would assert 0 == 3.
            Text = "Term 0 Term 1 Term 2 vocabulary seed.",
            CharStart = 0,
            CharEnd = 35,
            TokenEstimate = 8,
            CreatedAt = DateTimeOffset.UtcNow,
