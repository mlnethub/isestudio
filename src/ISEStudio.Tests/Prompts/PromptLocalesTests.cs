...
/// <see cref="PromptLocales"/> + <see cref="ISEStudioOptions.SystemLanguage"/>
...
public sealed class PromptLocalesTests
{
    ...
    // through the ISEStudioOptions config they now depend on.
    // ------------------------------------------------------------------
    ...
    {
        var svc = new TBoxExtractionService(Options.Create(new ISEStudioOptions { SystemLanguage = lang }));
    ...
    {
        var svc = new ABoxExtractionService(Options.Create(new ISEStudioOptions { SystemLanguage = lang }));
        var body = svc.ResolveSystemPrompt();
        Assert.False(string.IsNullOrWhiteSpace(body));
