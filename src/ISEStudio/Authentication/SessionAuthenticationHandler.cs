...
/// <summary>
/// Custom <see cref="AuthenticationHandler{TOptions}"/> for ISEStudio's
/// opaque-token server-side sessions. Reads the configured cookie name (the
/// default <c>isestudio_session</c> matches the Python backend), looks the
...
public sealed class SessionAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    private readonly IOptionsMonitor<ISEStudioOptions> _optionsMonitor;
    ...
    public SessionAuthenticationHandler(
        ...
        UrlEncoder encoder,
        TimeProvider clock,
        IOptionsMonitor<ISEStudioOptions> optionsMonitor)
    ...
    protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        var db = Context.RequestServices.GetRequiredService<ISEStudioDbContext>();
