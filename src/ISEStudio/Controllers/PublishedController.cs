...
/// </list>
/// Both surfaces set <c>X-ISEStudio-Release</c> + a quoted
...
public sealed class PublishedController : ControllerBase
{
    ...
    public PublishedController(IIntegrationApiFacade facade, ISEStudioDbContext db)
    {
        _facade = facade;
        _db = db;
    }
        ...
        CancellationToken ct)
    {
        ...
        // The X-ISEStudio-Release and ETag are set in lockstep so a
        ...
        Response.Headers["X-ISEStudio-Release"] = release.Version;
    ...
    /// enough to compute <c>X-ISEStudio-Release</c>, the ETag, and the
