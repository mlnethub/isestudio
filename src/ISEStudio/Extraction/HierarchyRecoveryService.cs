        ...
        return await Telemetry.LlmSource.WithLlmActivity(
            ...
            action: async ct =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    // the per-stage operationName ("Llm.TBoxHierarchy.HierarchyRecovery" /
                    // "Llm.TBoxHierarchy.HierarchyCritic"), then rethrow so
                    // the hierarchy-recovery pipeline sees a hard failure.
