...
public sealed class StructureAgent : IStructureAgent
{
    ...
    /// <see cref="ISEStudioOptions.AgenticIsolatedClasses"/> gate is off,
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            catch (Exception ex)
            {
                ...
                // swallowed → the class was left for a human with no
                // operator-visible signal. Log the diagnostic here, then
                // rethrow so the outer catch preserves its fail-soft return
                // null contract (matches the original Python
                // openrouter.chat_sync semantics).
