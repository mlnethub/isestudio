...
/// <summary>
/// Shared fixture that builds the production OnToPilot Docker image, starts
...
public sealed class ContainerSmokeFixture : IAsyncLifetime
{
    ...
    private readonly string _imageTag = $"ontopilot-smoke-backend:{Guid.NewGuid():N}";
    ...
    public ContainerSmokeFixture()
    {
        ...
        // src/OnToPilot.IntegrationTests/bin/<Configuration>/<Tfm>/.
        var location = AppContext.BaseDirectory;
        var cursor = new DirectoryInfo(location);
        while (cursor is not null && !File.Exists(Path.Combine(cursor.FullName, "OnToPilot.sln")))
        {
        ...
        if (cursor is null)
        {
            throw new InvalidOperationException(
                ...
                + "OnToPilot.sln must be reachable.");
        }
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            _network = new NetworkBuilder()
                .WithName($"ontopilot-smoke-{Guid.NewGuid():N}")
                .Build();

            _postgres = new PostgreSqlBuilder()
                .WithImage("postgres:16-alpine")
                .WithDatabase("ontopilot")
                .WithUsername("ontopilot")
                .WithPassword("ontopilot-smoke")
            ...
            _backend = new ContainerBuilder(_imageTag)
                ...
                .WithEnvironment(new Dictionary<string, string>
                {
                    ...
                    // EF Core binds from "OnToPilot:Persistence:ConnectionString"
                    // via the __ delimiter used by ASP.NET Core's
                    // EnvironmentVariablesConfigurationProvider.
                    ["OnToPilot__Persistence__ConnectionString"] = $"Host={PostgresAlias};Port=5432;Database=ontopilot;Username=ontopilot;Password=ontopilot-smoke",
                    ["OnToPilot__Storage__Endpoint"] = $"{MinioAlias}:9000",
                    ["OnToPilot__Storage__AccessKey"] = "minioadmin",
                    ["OnToPilot__Storage__SecretKey"] = "minioadmin-smoke",
                    ...
                    ["OnToPilot__LlmApiKey"] = "sk-or-v1-CANARY-SHOULD-NOT-LEAK",
                })
