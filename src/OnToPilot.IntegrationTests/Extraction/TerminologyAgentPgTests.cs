...
/// path is covered by
/// <c>src/OnToPilot.Tests/Extraction/TerminologyAgentOrchestrationTests.cs</c>;
...
/// mirrors <see cref="OnToPilot.IntegrationTests.Migration.IriSqlVerifierTests"/>'s
...
public sealed class TerminologyAgentPgTests : IAsyncLifetime
{
    ...
    private readonly PostgreSqlBuilder _builder = new PostgreSqlBuilder()
        ...
        .WithDatabase("ontopilot_term_pg")
    ...
    /// <c>src/OnToPilot/Program.cs:280-310</c>. Each
    /// <c>sp.GetRequiredService&lt;OnToPilotDbContext&gt;()</c>
    ...
    private ServiceProvider BuildServices(IChatClient chat)
    {
        ...
        services.AddSingleton<IDbContextFactory<OnToPilotDbContext>>(_ =>
        {
        ...
        // on the OnToPilot.Tests project's FakeChatClientFactory. The
    ...
    /// </summary>
    private sealed class PgDbContextFactory : IDbContextFactory<OnToPilotDbContext>
    {
        private readonly DbContextOptions<OnToPilotDbContext> _options;
        public PgDbContextFactory(DbContextOptions<OnToPilotDbContext> options) => _options = options;
        public OnToPilotDbContext CreateDbContext() => new(_options);
    }
    ...
    {
        var db = sp.GetRequiredService<OnToPilotDbContext>();
    ...
    public async Task SuggestAsync_persists_pending_proposal_on_postgres()
    {
        ...
        try
        {
            ...
            // affect the other.
            var factory = sp.GetRequiredService<IDbContextFactory<OnToPilotDbContext>>();
    ...
    // OnToPilot.IntegrationTests project does not need to reference
    // OnToPilot.Tests for FakeChatClientFactory + FakeChat).
    // ------------------------------------------------------------------

    /// <summary>
    /// Per-prefix monotonic legacy-id allocator. Mirrors the
    /// contract of <c>OnToPilot.Tests.Persistence.TestLegacyIds.Next</c>
