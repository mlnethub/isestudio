...
public sealed class AuthenticationContractTests
{
    ...
    public async Task Logout_clears_session_and_cookie()
    {
        ...
        var client = app.CreateClient();
        client.DefaultRequestHeaders.Add("Cookie", $"ontopilot_session={token}");

        var response = await client.PostAsync("/api/auth/logout", content: null);
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var setCookie = Assert.Single(response.Headers.GetValues("Set-Cookie"));
        Assert.Contains("ontopilot_session=", setCookie);
    ...
    public async Task Me_with_session_for_inactive_user_returns_user_inactive_detail()
    {
        ...
        var client = app.CreateClient();
        client.DefaultRequestHeaders.Add("Cookie", "ontopilot_session=inactive-token");
