...
namespace OnToPilot.Tests.EntityResolution;
...
public sealed class ResolutionServiceTests
{
    ...
    private static async Task<KnowledgeSystemEntity> CreateKsAsync(OnToPilotDbContext db, Guid ownerId, string tag)
    {
    ...
    private static async Task<EntityResolutionEntity> SeedRowAsync(
        OnToPilotDbContext db, Guid ksId, string surface, string status)
    {
