...
/// <see cref="OnToPilot.Tests.Observability.TelemetryTests"/> listener —
...
public sealed class CorpusHierarchyRecoveryIntegrationTests : IDisposable
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "ontopilot-recovery-" + Guid.NewGuid().ToString("N")[..12]);
        ...
        try
        {
            ...
            await using (var stream = new MemoryStream(Encoding.UTF8.GetBytes(Text)))
            {
                ...
                var verifyService = new TBoxVerifyService(Options.Create(new OnToPilotOptions()));
                var orchestrator = new ExtractionOrchestrator(
                    ...
                    corpus: new CorpusRecoveryService(
                        Options.Create(new OnToPilotOptions()), verifyService),
