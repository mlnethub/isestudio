...
/// (<see cref="OnToPilot.Infrastructure.Startup.StaleJobRecoveryService"/> and
/// <see cref="OnToPilot.Infrastructure.Startup.LegacyBackfillService"/>) are
...
/// the services by hand with a manual <see cref="OnToPilotDbContext"/>; they
    ...
    private sealed class StartupRecoveryHostFactory : WebApplicationFactory<Program>
    {
        ...
        public StartupRecoveryHostFactory()
        {
            var rawPath = Path.Combine(
                Path.GetTempPath(),
                $"ontopilot-startup-recovery-{Guid.NewGuid():N}.db");
        ...
        public PreSeedHandle PreSeed()
        {
            ...
            using var db = new OnToPilotDbContext(options);
