...
/// can also emit activities on the shared <c>OnToPilot.Rdf</c> source, and
...
public sealed class ActivityWrappingTests
{
    ...
    public async Task Storage_localcas_put_emits_activity_with_peer_service_and_outcome()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), "ontopilot-activity-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task Mcp_with_activity_helper_emits_activity_with_peer_service_and_outcome()
    {
        ...
        Assert.NotNull(activity);
        Assert.Equal("ontopilot.mcp", activity!.GetTagItem("peer.service"));
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "ontopilot-rdf-" + Guid.NewGuid().ToString("N"));
