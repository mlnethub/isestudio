...
public sealed class StructureAgentTests : IDisposable
{
    ...
    {
        _storePath = Path.Combine(Path.GetTempPath(), "ontopilot-structure-agent-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task Attach_adds_subclass_for_existing_parent()
    {
        ...
        var log = await RunAttachAsync(ksId, new OnToPilotOptions());
    ...
    public async Task Over_general_catch_all_is_left()
    {
        ...
        var log = await RunAttachAsync(ksId, new OnToPilotOptions { StructureMaxSameParent = 1 });
    ...
    public async Task Gate_off_never_calls_the_llm()
    {
        ...
        var log = await RunAttachAsync(ksId, new OnToPilotOptions { AgenticIsolatedClasses = false });
    ...
    public async Task Extraction_active_is_a_noop()
    {
        ...
        var log = await RunAttachAsync(ksId, new OnToPilotOptions(), jobs);
    ...
    public async Task Missing_llm_provider_is_a_noop()
    {
        ...
        var log = await RunAttachAsync(ks.Id, new OnToPilotOptions());
    ...
    public async Task Truncates_reason_to_200_chars()
    {
        ...
        await RunAttachAsync(ksId, new OnToPilotOptions());
