...
/// <see>OnToPilot.Integration.InternalOperationDispatcher</see> so
...
public sealed class TokenManagementService
{
    ...
    /// <summary>Default MCP token lifetime (minutes), driven by <c>OnToPilot:Mcp:TokenTtlMinutes</c>.</summary>
    public TimeSpan DefaultMcpLifetime =>
        TimeSpan.FromMinutes(_config.GetValue<int?>("OnToPilot:Mcp:TokenTtlMinutes") ?? 60);

    /// <summary>Maximum MCP token lifetime (minutes), driven by <c>OnToPilot:Mcp:MaxTokenTtlMinutes</c>.</summary>
    public TimeSpan MaxMcpLifetime =>
        TimeSpan.FromMinutes(_config.GetValue<int?>("OnToPilot:Mcp:MaxTokenTtlMinutes") ?? 24 * 60);
        ...
        Guid ksId, Guid userId, CancellationToken ct)
    {
        ...
        return new McpTokenListOut(
            Endpoint: _config["OnToPilot:Mcp:PublicUrl"] ?? string.Empty,
