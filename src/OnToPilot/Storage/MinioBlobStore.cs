...
public class MinioBlobStore : IBlobStore
{
    ...
    public Task<BlobWriteResult> PutAsync(Stream content, CancellationToken cancellationToken)
    {
        ...
        return Telemetry.StorageSource.WithStorageActivity(
            ...
            async ct =>
            {
                ...
                await hashing.DrainAsync(ct).ConfigureAwait(false);
    ...
    {
        var key = BlobKey.LegacyPathFor(sha256);
        return await Telemetry.StorageSource.WithStorageActivity(
            "storage.minio.get",
            null,
            async ct =>
            {
                try
                {
                    ...
                    return (Stream?)new ResponseOwningStream(response, response.ResponseStream);
                }
                catch (AmazonS3Exception ex) when (IsNotFound(ex))
                {
                    return null;
